...
namespace Board_Web.DotNetNote.Models
{
    public class NoteRepository
    {
        ...
        {
            return
                con.Query<string>("Select FileName From Notes Where Id = @Id", new { Id = Id }).SingleOrDefault();
        }
        ...
        public void UpdateDownCountById(int id)
        {
            ...
            con.Execute("Update Notes Set DownCount = DownCount + 1" + " Where FileName = @FileName", p, commandType: CommandType.Text);
        }
        ...
        {
            string sql = $"SELECT TOP {numberOfNotes} Id, Title, Name, PostDate FROM Notes Order By Id Desc";
            return con.Query<Note>(sql).ToList();
        }
